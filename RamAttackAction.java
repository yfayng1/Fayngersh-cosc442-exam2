...
public class RamAttackAction extends AbstractAttackAction {
     ...
     */
    public ToHitData toHit(IGame game, MovePath mPath) {
        final Entity ae = game.getEntity(getEntityId());
        final Targetable target = getTarget(game);
        Coords ramSrc = ae.getPosition();
        int ramEl = ae.getElevation();
        Coords priorSrc = mPath.getSecondFinalPosition(ae.getPosition());
        MoveStep ramStep = null;

        // let's just check this
        if (!mPath.contains(MovePath.STEP_RAM)) {
            return new ToHitData(TargetRoll.IMPOSSIBLE,
                    "Ram action not found in movement path");
        }

        // determine last valid step
        mPath.compile(game, ae);
        for (final Enumeration<MoveStep> i = mPath.getSteps(); i.hasMoreElements();) {
